...
def check_server_health() -> bool:
    ...
    try:
        ...
        print(f"   Command: docker-compose -f docker-compose.scale.yml up -d --scale api=3")
        return False
...
def test_load_balancing_with_rapid_requests(examples_df: pd.DataFrame):
    ...
    print(f"   🏠 Making 10 rapid requests for: {house_data['bedrooms']} bed, {house_data['bathrooms']} bath, {house_data['sqft_living']:,} sqft")
    ...
    for i in range(10):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"   Request {i+1:2d}: ${result['predicted_price']:,.0f} - {response_time:.0f}ms")
            else:
    ...
    if response_times:
        ...
        if len(unique_predictions) == 1:
            ...
            print(f"      ⚠️  Prediction variance detected: {len(unique_predictions)} different values")
...
def test_batch_prediction_scaling(examples_df: pd.DataFrame) -> dict:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"      Average per house: {processing_time/len(result['predictions']):.0f}ms")
...
def test_concurrent_users_simulation(examples_df: pd.DataFrame):
    ...
    if successful:
        ...
        if failed:
            ...
            for fail in failed:
                error_msg = fail.get('error', f"Status {fail.get('status_code', 'unknown')}")
...
def main():
    ...
    if not check_server_health():
        ...
        print("   Command: docker-compose -f docker-compose.scale.yml up -d --scale api=3")
        sys.exit(1)
