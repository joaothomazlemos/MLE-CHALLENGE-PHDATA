...
class ModelService:
        ...
        try:
            if not self.demographics_path.exists():
                raise ModelLoadError(
                    f"Demographics file not found: {self.demographics_path}"
                )

            self._demographics = pd.read_csv(
                self.demographics_path, dtype={"zipcode": str}
            )
    ...
    def get_demographics_for_zipcode(self, zipcode: str) -> Optional[Dict]:
        ...
        demographic_row = self._demographics[self._demographics["zipcode"] == zipcode]
    ...
    def health_check(self) -> Dict:
        ...
        try:
            ...
            demographics_loaded = (
                self._demographics is not None and not self._demographics.empty
            )
            ...
            healthy = all(
                [model_loaded, features_loaded, demographics_loaded, prediction_works]
            )
            ...
            return {
                ...
                "zipcodes_count": (
                    len(self._demographics) if self._demographics is not None else 0
                ),
            }
