...
class HouseFeatures(BaseModel):
    ...
    sqft_living: int = Field(
        ..., ge=100, le=15000, description="Square feet of living space"
    )
    ...
    waterfront: int = Field(
        ..., ge=0, le=1, description="Waterfront property (0=No, 1=Yes)"
    )
    ...
    sqft_above: int = Field(
        ..., ge=100, le=15000, description="Square feet above ground"
    )
    sqft_basement: int = Field(
        ..., ge=0, le=5000, description="Square feet of basement"
    )
    yr_built: int = Field(..., ge=1800, le=2025, description="Year built")
    yr_renovated: int = Field(
        ..., ge=0, le=2025, description="Year renovated (0 if never)"
    )
    ...
    sqft_living15: int = Field(
        ..., ge=100, le=15000, description="Living space of nearest 15 neighbors"
    )
    sqft_lot15: int = Field(
        ..., ge=500, le=200000, description="Lot size of nearest 15 neighbors"
    )
...
class MinimalHouseFeatures(BaseModel):
    ...
    waterfront: int = Field(
        0, ge=0, le=1, description="Waterfront property (0=No, 1=Yes)"
    )
    ...
    lat: Optional[float] = Field(
        None, ge=47.0, le=48.0, description="Latitude (auto-filled from zipcode)"
    )
    long: Optional[float] = Field(
        None, ge=-123.0, le=-121.0, description="Longitude (auto-filled from zipcode)"
    )
    sqft_living15: Optional[int] = Field(
        None, ge=100, le=15000, description="Living space of nearest 15 neighbors"
    )
    sqft_lot15: Optional[int] = Field(
        None, ge=500, le=200000, description="Lot size of nearest 15 neighbors"
    )
...
class PredictionMetadata(BaseModel):
    ...
    model_metadata: dict = Field(
        ..., description="Metadata of the ML model used, from ModelService"
    )
    prediction_timestamp: str = Field(..., description="ISO timestamp of prediction")
    zipcode_demographics_available: bool = Field(
        ..., description="Whether demographics data was found for zipcode"
    )
...
class PredictionResponse(BaseModel):
    ...
    predicted_price: float = Field(
        ..., ge=0, description="Predicted house price in USD"
    )
...
class BatchPredictionRequest(BaseModel):
    ...
    houses: list[HouseFeatures] = Field(
        ..., min_items=1, max_items=100, description="List of houses to predict"
    )
