...
def price_range_analysis(X, y, model):
    ...
    ranges = [
        (
            "Low Price",
            low_price,
            f"${y[low_price].min():,.0f} - ${y[low_price].max():,.0f}",
        ),
        (
            "Mid Price",
            mid_price,
            f"${y[mid_price].min():,.0f} - ${y[mid_price].max():,.0f}",
        ),
        (
            "High Price",
            high_price,
            f"${y[high_price].min():,.0f} - ${y[high_price].max():,.0f}",
        ),
    ]
    ...
    for name, mask, price_range in ranges:
        if mask.sum() > 0:
            ...
            print(
                f"   • {name:10} ({mask.sum():,} houses): MAE=${range_mae:,.0f}, R²={range_r2:.3f}, Error={range_mape:.1f}%"
            )
...
def generate_business_recommendations(gen_results, fit_results):
    ...
    if gen_results["generalization"] in ["Excellent", "Good"]:
        ...
        recommendations.append(
            "⚠️ Consider collecting more training data before deployment"
        )

    if fit_results["model_fit"] in ["Overfitting"]:
        recommendations.append(
            "⚠️ The model may be too complex - consider simpler approaches"
        )
    ...
    if fit_results["performance"] in ["Good", "Excellent"]:
        ...
        recommendations.append(
            f"📊 Expect typical prediction errors of ~${gen_results['test_mae']:,.0f}"
        )
    else:
        recommendations.append(
            "⚠️ Consider feature engineering or different algorithms for better accuracy"
        )

    error_pct = gen_results["test_mape"]
    if error_pct < 12:
        recommendations.append(
            f"✅ {error_pct:.1f}% average error is excellent for real estate"
        )
    elif error_pct < 20:
        recommendations.append(
            f"✅ {error_pct:.1f}% average error is acceptable for real estate"
        )
    else:
        recommendations.append(
            f"⚠️ {error_pct:.1f}% average error may be too high for some use cases"
        )
...
def main():
    ...
    print(
        f"💰 Expected Error: ${gen_results['test_mae']:,.0f} ({gen_results['test_mape']:.1f}%)"
    )
